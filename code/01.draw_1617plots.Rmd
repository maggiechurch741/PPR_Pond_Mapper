...
**Overview**: We have a shapefile containing the plot boundaries for the areas that HAPET typically surveys annually. However, the 2016-2017 dataset surveyed additional plots that aren't usually surveyed. We needed to manually draw the boundaries for those bonus 2016-2017 plots.

**Inputs:**

-   Raw HAPET plot boundaries - these are permanent plots that are re-surveyed most years

-   Raw HAPET 2016-2017 pond shapefiles

**Steps:**

1.  Open an RShiny app: opens a map of the 2016-2017 delineated ponds and logs the coordinates for rectangles drawn within the app. I use this to eyeball plot boundaries.

2.  Manually create 2016-2017 plot polygons from those coordinates

3.  Combine 2016-2017 plot polygons into an sf object and assign a plot id

4.  Combine these hand-drawn 2016-2017 plots with the permanent plot boundaries

**Outputs:**

-   Shapefile of clean plot boundaries ("allPlots.shp")
...
This step is not reproducible, it just documents my steps (skip this chunk). Note, plot boundaries are conservative approximations, excludes water at plot edges. That's ok.
```{r, eval=F}
ui <- fluidPage(
