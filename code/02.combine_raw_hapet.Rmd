...
for (i in seq_along(datasets)) {
  ...
  out[[i]] <- purrr::map_dfr(unique(ponds_yr$Plot), function(pid) {
    
    ponds_plot <- ponds_yr |>
      filter(Plot == pid) |>
      st_make_valid() |>                          # fix invalid geometries
      subset(!st_is_empty(geometry))              # drop any that become invalid after fixing
    
    # touching polygons
    true_touching <- ponds_plot |>
      filter(lengths(st_touches(ponds_plot)) > 0)
    
    if (nrow(true_touching) > 0) {
      touch_graph <- st_touches(true_touching)
      g <- graph_from_adj_list(touch_graph)
      components <- components(g)$membership
      
      hapet_touch_unioned <- true_touching |>
    ...
    # non-touching
    true_non_touching <- ponds_plot |>
      filter(lengths(st_touches(ponds_plot)) == 0) |>
      select(Plot, dataset, geometry)
    
    # combine
    bind_rows(hapet_touch_unioned, true_non_touching)
  })
}
