...
needs_plots <- allPonds %>% 
  filter(dataset %in% c("pair16", "brood16", "pair17", "brood17")) %>% 
  st_transform(4326)
```

Open a UI to streamline plot delineation
```{r, eval=F}
ui <- fluidPage(
...
Add plot coordinates from the UI here
```{r}
# plot
p1 <- list(matrix(c(
...
max_hapet_plotid <- allPonds %>% st_drop_geometry() %>% summarize(max(Plot, na.rm=T)) %>% pull()
...
plots <- lapply(polygons_list, function(p) p[[1]]) %>%
  st_sfc(crs = 4326) %>% 
  st_sf(Plot = plot_ids,
        geometry = .) 
```

Write over allPonds, filling in the new pond ids. Some small ponds near plot boundaries didn't get assigned a plot, but that's ok. 
```{r}
# add plot id to 2016-2017 datasets
join <- st_join(needs_plots, plots) %>%
  rename(Plot=Plot.y) %>%
  select(-Plot.x) 

# some ponds span multiple plots - split these ponds by plot boundary
dup <- join %>% 
  group_by(rownum) %>%
  filter(n()>1) %>% 
...
# add dups back to pond list. And reproject to UTM14N
ponds_1617 <- join %>% 
  group_by(rownum) %>%
  filter(n()==1) %>%
  bind_rows(dup) %>% 
...
# add all ponds + their new plot ids back to "allPonds"  
allPonds_new <- allPonds %>% 
  filter(!dataset %in% c("pair16", "brood16", "pair17", "brood17")) %>%
  bind_rows(ponds_1617)

# save over allPonds
st_write(allPonds_new, "../data/allPonds/allPond_fixed.shp", append=FALSE)
```

Write plot boundaries to file
```{r}
st_write(plots, "../data/intermediate/plots1617/plots_1617.shp", append=FALSE)
```
