...
# Sample wet points
Sample wet GT points: 100 wet points from each plot (and at least 1 point per pond)
...
sample_points_per_plot <- function(plot_polygons) {
  ...
  if (n_polygons >= 100) {
    # If 100+ polygons, sample one point per polygon
    sampled_points <- map_dfr(plot_polygons$geometry, ~ sample_points(.x, 1))
  } else {
    # If fewer than 100 polygons, distribute extra points
    extra_points_needed <- 100 - n_polygons
