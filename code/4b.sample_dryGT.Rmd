...
**Description**: Limit the Possibility Layer to known dry areas. We will sample dry points from within this area, to train the model what is dry.

**Inputs**:

-   HAPET ponds (output of `1.combineRawHapet.Rmd`)

-   Plot-level rasters of the possibility layer (output of `2.process_possibility_rasters.Rmd`)

-   Plot-level rasters of the permanent lakee layer (output of `2.process_permanent_lakes.Rmd`)

-   State boundaries

**Output**: shapefile of sampled dry points, keeping attributes.

**Steps:** For a given plot:

1.  load possibility raster for that plot

    ![](images/1.poss.png){width="400"}

2.  remove edges

    ![](images/2.poss_remove_edges.png){width="400"}

3.  get ponds

    ![](images/3.ponds.png){width="400"}

4.  get permanent water

    ![](images/4.permWater.png){width="400"}

5.  mask ponds and permanent water from possibility layer

    ![](images/5.dryROI.png){width="400"}

6.  Sample points within this dry ROI

    ![](images/6.sampled_points.png){width="400"}
...
allPonds <- st_read("../data/allPonds/allPond_fixed.shp")

# apply 15m outward buffer to ponds
ponds_outbuff75 <- st_buffer(allPonds, 7.5)
```

Remove known wet areas from possibility layer -\> sample dry points.
...
sample_dry <- function(p, d) {
  ...
  perm_i <- perm_file %>%
      rast() %>%        
      resample(poss_i)  # Snap to poss_i
...
# List raster files
cropped_poss_files <- list.files(paste0(temp_dir, "/cropped_poss_rasters"), full.names = TRUE) 
cropped_permWater_files <- list.files(paste0(temp_dir, "/cropped_permWater_rasters"), full.names = TRUE) 
...
st_write(dryPoints_sf_wstate, "../data/sampled_points/dry.shp", append=FALSE)
```
