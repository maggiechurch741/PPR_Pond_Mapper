...
testing_task <- testing_task$select(sel_featureset)
...
confusion_by_group(test_results, sz_class)
...
pred21_1 <- rast("~/Downloads/pred_image_ppr_21-0000006400-0000044800.tif") %>% 
  subset(c("blue", "green", "red"))  

# 41m pixels and 13 bands
dim(pred21_1)
...
r_df <- pred21_1 %>% as.data.frame(xy=T)
  
ggplot(r_df, aes(x = x, y = y)) +
  geom_raster(aes(fill = rgb(red, green, blue))) +
  scale_fill_identity() + 
  coord_fixed() + 
  theme_minimal()
```
