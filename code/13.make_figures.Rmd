...
# Train vs Test data
```{r}
test_years <- c("pair16", "brood16", "2019")

test_plots <- testing %>% 
  st_drop_geometry() %>%
  filter(!(dataset %in% test_years)) %>% 
  distinct(plot_id) %>% 
  pull() 

train_plots <- training %>% 
  st_drop_geometry() %>%
  distinct(plot_id) %>% 
  pull() 

mapview(plots %>% filter(Plot %in% test_plots), alpha = 1, col.regions="blue", color="blue", lwd=3, legend=F) + 
  mapview(plots %>% filter(Plot %in% train_plots), alpha = 1, col.regions="orange", color="orange", lwd=3, legend=F) + 
