...
This is a script for creating models from processed tidy data
...
training <- read_csv('../data/balanced_training_b100.csv') %>%
...
testing <- testing %>% filter(!(area_cr < 2 & dataset == "2020")| type=="dry")
training <- training %>% filter(!(area_cr < 2 & dataset == "2020")| type=="dry")
```


3. Create CV folds, using k-means coordinate clustering

```{r}
set.seed(123)

# Function to apply kmeans to each group
apply_kmeans_per_group <- function(group_data) {
  
  # Extract coordinates
  coords <- st_coordinates(group_data)
  
  # Apply kmeans clustering
  kmeans_result <- kmeans(coords, centers = 5, algorithm = "Lloyd", iter.max = 60)

  # Add the cluster assignments back to the data
  group_data <- group_data %>% mutate(clusterid = kmeans_result$cluster)
  
  return(group_data)
}

# Apply kmeans clustering to each group within 'training' grouped by 'dataset'
clustered_training <- training %>%
  group_by(dataset) %>%
  group_modify(~ apply_kmeans_per_group(.x)) %>%
  ungroup() %>% 
  # Actually I want 2017 to be its own, single cluster
  mutate(clusterid = ifelse(dataset %in% c("pair17", "brood17"), 1, clusterid)) 

# add "block" var, to get cluster-dataset blocks. We'll use these as CV folds
dat <- clustered_training %>% 
  mutate(block = as.factor(paste(clusterid, dataset))) 
```
